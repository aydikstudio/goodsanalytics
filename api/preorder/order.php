    ...
    foreach ($cellIterator as $cell) {
        ...
        if($cell != "name" && $cell != "phone") {
            if($arr_index == 0) {
                ...
                $arr_item["will_postavlka_count"] = $value;
                $arr_index = 0;
                $ready = 1;
            }
        }
...
if(fwrite($f_hdl,$json)) {
        $query ="UPDATE `downloaded_status` SET `status`=0 WHERE `name`= 'preorder'";
            $result = mysqli_query($mysqli, $query) or die("Ошибка " . mysqli_error($mysqli)); 
            if(!$result) {
                echo "Ошибка";
            }
...
function group_cell($arr) {
    ...
    foreach ($arr as $row) {
        $id = $row['id'];
        $will_postavlka_count = $row['will_postavlka_count'];
        $pp = 0;
        ...
        if (!@array_key_exists($id, $aggregated)) {
            ...
            $aggregated[$id] = [
            'name' => trim($id),
               'will_postavlka_count' => trim($will_postavlka_count)
            ];
    
            continue;
        }
        
        $aggregated[$id]['will_postavlka_count'] += $will_postavlka_count;
    }
